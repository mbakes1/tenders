/*
  ...
  1. New Columns
    - `view_count` (integer) - cached count of total views for performance

  2. New Tables
    - `tender_views` - detailed view tracking with IP, user agent, user ID, timestamp

  3. Functions
    - `increment_tender_view` - safely increment view count with spam prevention
    - `get_tender_view_stats` - get detailed view statistics
    - `get_popular_tenders` - get most viewed tenders

  4. Security
    - RLS enabled with public read access for view counts
    - Spam prevention (1 view per IP per hour per tender)
    - Service role can manage all view data

  5. Indexes
    - Performance indexes for view queries
*/
